...
def duplicate_row_removal(df1,df2):
...
def year_analysis(country,df):
    ...
    fig,ax = plt.subplots()
    ax.plot(filtered_df['Year'].values,filtered_df['Gold'].values,color='gold',label='GOLD',marker='o',linestyle='-')
    ax.plot(filtered_df['Year'].values,filtered_df['Silver'].values,color='silver',label='SILVER',marker='o',linestyle='-')
    ax.plot(filtered_df['Year'].values,filtered_df['Bronze'].values,color='brown',label='BRONZE',marker='o',linestyle='-')
    ax.plot(filtered_df['Year'].values,filtered_df['Total_Medal'].values,color='green',label='TOTAL_MEDALS',marker='o',linestyle='-')
    ax.legend()
    st.pyplot(fig)